...
#if defined(NDEBUG)
# error "snowflakes cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x637058781107629bd47ea45ba7b73bbf434b523f41f7ce132b2e4d957f75d2d2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // snowflakes: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "snowflakes Signed Message:\n";
...
{
    // snowflakes: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	else if(nHeight < 8888)  
    {
        nSubsidy = 1 * COIN;
    }
    else if(nHeight < 17776)  
    {
        nSubsidy = 0.9 * COIN;
    }
    else if(nHeight < 26664)  
    {
        nSubsidy = 0.8 * COIN;
    }
    else if(nHeight < 35552)  
    {
        nSubsidy = 0.7 * COIN;
    }
    else if(nHeight < 44440)  
    {
        nSubsidy = 0.6 * COIN;
    }
    else if(nHeight < 53328)  
    {
        nSubsidy = 0.5 * COIN;
    }
    else if(nHeight < 62216)  
    {
        nSubsidy = 0.4 * COIN;
    }
    else if(nHeight < 71104)  
    {
        nSubsidy = 0.3 * COIN;
    }
    else if(nHeight < 79992)  
    {
        nSubsidy = 0.2 * COIN;
    }
    else if(nHeight < 88880)  
    {
        nSubsidy = 0.1 * COIN;
    }
    else if(nHeight < 97768)  
    {
        nSubsidy = 0.09 * COIN;
    }
    else if(nHeight < 106656)  
    {
        nSubsidy = 0.08 * COIN;
    }
    else if(nHeight < 115544)  
    {
        nSubsidy = 0.07 * COIN;
    }
    else if(nHeight < 124432)  
    {
        nSubsidy = 0.06 * COIN;
    }
    else if(nHeight < 133320)  
    {
        nSubsidy = 0.05 * COIN;
    }
    else if(nHeight < 142208)  
    {
        nSubsidy = 0.04 * COIN;
    }
    else if(nHeight < 151096)  
    {
        nSubsidy = 0.03 * COIN;
    }
	else if(nHeight < 159984)  
    {
        nSubsidy = 0.02 * COIN;
    }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // snowflakes: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // snowflakes: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x637058781107629bd47ea45ba7b73bbf434b523f41f7ce132b2e4d957f75d2d2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 2016-12-03 : Why It Will Be Hard to Repeal Obamacare";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("05db730f71c533147e95262049f991d8a2c7cb907eb11f7081b22d897eb38e5956f67f6b8bc94dbd90ef656033c32dfd9b08fc9164d70a78c83d039647be621335") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1480803157;
        ...
        assert(block.hashMerkleRoot == uint256("0x49819930b3ea457f4d4b6f60403be949296874850aa5244200d2c8aa42c99f71"));
...
unsigned char pchMessageStart[4] = { 0xc4, 0x40, 0xb9, 0xb3 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("snowflakesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("snowflakesMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static snowflakesMiner(CWallet *pwallet)
{
    printf("snowflakesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("snowflakes-miner");
    ...
    try { loop {
        ...
        printf("Running snowflakesMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("snowflakesMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&snowflakesMiner, pwallet));
}
